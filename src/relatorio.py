...
class Relatorio:
    ...
    def buscar_pomodoros(self):
        try:
            data_limite = (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d") #Busca nos ultimos 7 dias
            self.cursor.execute("""
    ...
    #Faz uma busca e a statisica dos pomodoros armazenados
    def buscar_estatisticas(self):
        try:
            ...
            #Faz a estatisca do tempo total estudado
            self.cursor.execute("""
    ...
    #Exibe as estatisticas na interface
    def atualizar_interface(self, treeview, label_estatisticas=None):
        try:
            ...
            registros = self.buscar_pomodoros_da_semana()
